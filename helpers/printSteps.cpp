...
split(const std::string &s, const std::string &delimiter, const bool &removeEmptyEntries = false) {
...
void printCommands(std::string fileName) {
    ...
    for (unsigned int i = 0; i < commands.size(); i++) {
        if (commands[i].line > 0) {
            ...
            Colors colorToUse = (useColor || commands[i].colOffset > 0) ? static_cast<Colors>(
                    static_cast<int>(Colors::RED) + commands[i].colOffset) : Colors::NONE;
